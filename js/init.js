...
function grax_tm_contact_form() {
	...
	const emptyNotice = form.querySelector('.empty_notice');
	const submitBtn = form.querySelector('button[type="submit"]');
	const toast = document.getElementById('contact_toast');
	...
	const showToast = (text, isError = false) => {
		...
		toast.hideTimeout = setTimeout(() => toast.classList.remove('show'), 4000);
	};
	...
	form.addEventListener('submit', function (e) {
		...
		if (!name || !email || !message) {
			if (emptyNotice) {
				...
				setTimeout(() => emptyNotice.classList.remove('show'), 2000);
			}
			return;
		}
		...
		fetch('https://formsubmit.co/ajax/raymondamoateng@gmail.com', {
			method: 'POST',
			body: formData
		}).then(response => {
			if (!response.ok) {
			...
			form.reset();
			showToast('Message sent. Thank you for reaching out, I\'ll respond shortly.');
			showMessage('Message sent! Please check your inbox for my reply.', true);
		}).catch(() => {
			showToast('Something went wrong. Please email me directly.', true);
			showMessage('Something went wrong. Please email me directly.', false);
		}).finally(() => {
			if (submitBtn) {
...
function grax_tm_switcher_opener() {
	...
	// Initialize active state based on body class
	if (jQuery('body').hasClass('dark')) {
		direction.removeClass('active');
		dark.addClass('active');
		settings.addClass('changed');
	} else {
		direction.removeClass('active');
		light.addClass('active');
		settings.removeClass('changed');
	}
